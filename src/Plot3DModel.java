...
/**
 * Manages the data model for 3D plotting, including functions, grid points, and value ranges
 */
public class Plot3DModel {
    ...
    private void calculateFunctionValues(Function3DInfo functionInfo, double xMin, double xMax, 
        ...
        if (functionInfo.gridPoints == null || 
            functionInfo.gridPoints.length != resolution || 
            functionInfo.gridPoints[0].length != resolution) {
            
            functionInfo.gridPoints = new Plot3DPoint[resolution][resolution][3];
        }
        ...
        for (int i = 0; i < resolution; i++) {
            ...
            for (int j = 0; j < resolution; j++) {
                ...
                functionInfo.gridPoints[i][j][0] = new Plot3DPoint(x, y, z);
                
                // Also initialize transformed and projected points as copies
                // They'll be updated later by the transformer
                functionInfo.gridPoints[i][j][1] = new Plot3DPoint(x, y, z);
                functionInfo.gridPoints[i][j][2] = new Plot3DPoint(x, y, 0); // Z=0 for projection
        ...
        if (localZMin < zMin) zMin = localZMin;
        if (localZMax > zMax) zMax = localZMax;
    }
    ...
    private void recalculateZRange() {
        ...
        for (Function3DInfo functionInfo : functions) {
            if (functionInfo.gridPoints != null) {
                int resolution = functionInfo.gridPoints.length;
                
                for (int i = 0; i < resolution; i++) {
                    for (int j = 0; j < resolution; j++) {
                        if (functionInfo.gridPoints[i][j][0] != null) {
                            double z = functionInfo.gridPoints[i][j][0].getZ();
    ...
    public static class Function3DInfo {
        ...
        Plot3DPoint[][][] gridPoints; // [x][y][phase] where phase is 0=original, 1=transformed, 2=projected
